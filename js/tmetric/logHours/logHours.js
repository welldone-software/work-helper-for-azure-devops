      ...
      chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, { url: tabUrl }) => {            
        ...
        chrome.tabs.executeScript(trackerTabId, {
          code: `dateToLogLocaleDateString = '${dateToLogLocaleDateString}';`,
            ...
            chrome.tabs.executeScript(trackerTabId, { file: 'js/tmetric/logHours/content_scripts/logHours.js' });
        });
...
const checkDidAlarmTimePassToday = alarmTime => {
  ...
  const totalSecondsToday = ((today.getHours() * 360) + (today.getMinutes() * 60) + today.getSeconds());
  const totalAlarmTimeSeconds = ((alarmTime.hours * 360) + (alarmTime.minutes * 60) + alarmTime.seconds);
